...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const calendarMsg = calendarCreated
    ? '<p style="color:#16a34a;font-weight:600;">✅ Google Calendar 일정이 자동 생성되었습니다.</p>'
    : calendarError
    ? `<p style="color:#dc2626;">⚠️ 캘린더 일정 생성 실패: ${calendarError}</p><p style="color:#6b7280;font-size:13px;">Admin 대시보드에서 재시도할 수 있습니다.</p>`
    : '<p style="color:#6b7280;font-size:13px;">Google Calendar가 설정되지 않았습니다.</p>';

  return res.status(200).send(resultPage(
    '승인 완료',
    `<strong>${companyName}</strong>의 미팅 신청이 승인되었습니다.<br><br>
...
<head>
  ...
  <style>
    ...
    .msg p{margin:4px 0;}
    .badge{display:inline-block;padding:4px 12px;border-radius:20px;font-size:13px;font-weight:600;background:${c.bg};color:${c.border};border:1px solid ${c.border};}
